...
const createAndInsertCoinDeskCoinsList = async () => {
    ...
    for (let i=0; i<coindesk_coins_list.length; i++){
        coindesk_coins_list_insert_db.push(
            [
                coindesk_coins_list[i]["Symbol"],
                coindesk_coins_list[i]["Name"],
                coindesk_coins_list[i]["Sector"]
            ]
        )
    }
    await insert_to_db_table("categories_coins_list", coindesk_coins_list_insert_db);
}
