...
// 자동화 함수 : 1d
export const update_queries_1d = async (req, res) => {
    let data_1d=[]
    let last=[];
    let now_timestamp=0;
    let value=0;
    ...
    try {
         ...
         data_1d = await select_data("SNPCMC_1d") //쿼리 선택 후 불러오기
         console.log("try succeeded");
         last= data_1d.slice(-1)[0] //가장 최근 데이터 값
         console.log("last",last);
    } catch (error) {
        console.log(error);
        res.json({ message: error.message });        
    ...
    if (value>=86400){
        await delete_data("SNPCMC_1d")
        let update_data_1d=[]; //쿼리에 최종적으로 들어가기 위한 데이터
        let data_1d=[];
        data_1d=await getData1mo(); //API 데이터 불러오기
        for (let i=0; i<data_1d.length; i++){
            update_data_1d.push([
                data_1d[i].time,
                data_1d[i].SnP,
        ...
        await insert_to_db_table("SNPCMC_1d",update_data_1d); //쿼리에 데이터 넣기
